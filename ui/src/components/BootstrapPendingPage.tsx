  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? "A bootstrap invite is already active. Check your Lovon Teams startup logs for the first-admin URL, or run this command on the host to rotate it:"
          : "Run this command on the host that runs Lovon Teams to print a one-time first-admin invite URL:"}
      </p>
...
export function BootstrapPendingPage({
  ...
  if (!claimAvailable) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">This Lovon Teams is waiting on its first admin</h1>
        <p className="mt-2 text-sm text-muted-foreground">
  ...
  if (!session) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">Finish setting up this Lovon Teams</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          No admin has claimed this instance yet. Sign in or create your Lovon Teams account to become the first
