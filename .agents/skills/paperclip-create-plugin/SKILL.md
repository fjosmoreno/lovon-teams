---
name: lovon-create-plugin
description: >
  Create and develop external Lovon Teams plugins with the CLI-first workflow.
  Use when scaffolding a new plugin, working on a local plugin against a running
  Lovon Teams instance, or updating plugin authoring docs. Covers `paperclipai
...
# Create and develop a Lovon Teams plugin

Use this skill when the task is to create, scaffold, or iterate on a Lovon Teams plugin against a local Lovon Teams instance.

## 1. Default: build the plugin OUTSIDE Lovon Teams core
...
- Scaffold the plugin into a directory outside the Lovon Teams checkout (e.g. `~/dev/paperclip-plugins/<name>`).
- Install it into the running Lovon Teams instance by local absolute path.
- Edit code in the external package; let Lovon Teams pick up rebuilt output.

Only edit Lovon Teams core itself when the user asks to surface a plugin as a bundled example (`server/src/routes/plugins.ts`, in-repo example lists, docs).
...
- `--sdk-path <path>` — snapshot the local SDK from a Lovon Teams checkout into `.paperclip-sdk/` (useful when developing against an unreleased SDK).
...
If you changed Lovon Teams SDK/host/plugin runtime code in addition to the plugin, also run the relevant Lovon Teams workspace checks.
...
## 8. When NOT to edit Lovon Teams core
