---
name: lovon-converting-plans-to-tasks
description: >
  The Lovon Teams way of converting a plan into executable tasks. Use whenever
  you are asked to plan, scope, or break down work inside a Lovon Teams company.
  Industry-agnostic guidance on how to translate a plan into assigned issues
  with the right specialty, dependencies, and parallelization so Lovon Teams's
  executor can pick up the work — it does not prescribe a plan format. Pair
  with the `lovon` skill, which covers the mechanics of writing the plan
  document and reassigning the issue.
---

# Lovon Teams — Converting Plans to Tasks

A companion skill for turning a plan into executable Lovon Teams work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Lovon Teams works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `lovon` skill. This skill covers planning method, not the API surface.
...
- Not a replacement for the `lovon` skill's planning mechanics. Use both.
