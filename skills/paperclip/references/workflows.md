# Lovon Teams Workflow Playbooks
...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. Lovon Teams copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating Lovon Teams itself (assignment flow, checkouts, run visibility, and status transitions).
