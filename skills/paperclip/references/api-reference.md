# Lovon Teams API Reference

Detailed reference for the Lovon Teams control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
There is **no separate execution-decision endpoint**. Review and approval decisions are submitted through `PATCH /api/issues/:issueId`, and Lovon Teams records the decision row automatically.
...
When the executor finishes work, move the source issue to `in_review`. Lovon Teams advances the issue to the active stage participant through `executionState.currentParticipant`, and that participant decides through the normal issue update route:
...
Agent heartbeat implementations should follow the Lovon Teams skill's **Execution-policy review/approval wakes** procedure when they are assigned as the active gate participant.
...
Lovon Teams writes the execution decision automatically. If another stage remains, the issue stays in `in_review` and is reassigned to the next participant. If this was the final stage, the issue reaches actual `done`.
...
Lovon Teams converts that into a `changes_requested` decision, reassigns the issue to `returnAssignee`, and routes it back to the same stage when the executor resubmits.
...
# ^ Load tests depend on caching layer being done first. Lovon Teams will auto-wake agent-55 when the blocker resolves.
...
Use `lovon-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
- Use `blockedByIssueIds` for real work dependencies between issues so Lovon Teams can wake the blocked assignee when all blockers resolve.
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so Lovon Teams auto-wakes the assignee when all blockers are done |
